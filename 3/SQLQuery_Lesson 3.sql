...
-- Class Work # 3     07.12.19

--   like  -  ищем что-то подобное
...
values ('admin1', 'qqqq', 'wert', 'trewq', 'zzzzz', CURRENT_TIMESTAMP, '2000-01-01', 'D')
go

insert into UserData (UserName, [Password], FirstName, MiddleName, LastName, CreateDate, UpdateDate, [Status]) 
select 'admin2', 'qqqq', 'wert', 'trewq', 'zzzzz', CURRENT_TIMESTAMP, '2000-01-01', 'D'
go

insert into UserData (UserName, [Password], FirstName, MiddleName, LastName, CreateDate, UpdateDate, [Status]) 
select 'admin3', 'qqqq', 'wert', 'trewq', 'zzzzz', CURRENT_TIMESTAMP, '2000-01-01', 'D'
go


select (select g.UserGroupId
from UserGroup g
where g.Name = 'Группа №2. Группа буднего дня. Старт 09.12.2014'), (select * from getUserId('admin2'))
go
...
select *
from getUserId('admin2')
go



select d.UserDataId
from UserData dselect *
from UserGroupLink l
where l.UserDataId in (select d.UserDataId
                        from UserData d
						where d.UserName in ('admin1', 'admin2', 'admin3'))
where d.UserName in ('admin1', 'admin2', 'admin3')
go

select *
from UserData u
inner join UserGroupLink l
on u.UserDataId = u.UserDataId
inner join UserGroup p on l.UserGroupId = p.UserGroupId
go
...
update UserData set [Status] = 'A', UpdateDate = CURRENT_TIMESTAMP
...
delete
from UserGroupLink
where UserDataId in (select d.UserDataId
                        from UserData d
						where d.UserName in ('admin1', 'admin2', 'admin3'))
go
